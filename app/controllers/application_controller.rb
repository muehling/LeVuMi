    ...
    test.each do |t|
      ...
      items.each do |i|
        if (t[1] == 'SW2' && i[2] == 'gerade') ||(t[1] == 'SEL4' && i[2] == 'In') || (t[1] == 'SL2a' && (i[2] == 'Ra' ||i[2] == 'ra')) ||
            ...
            (i[2] == 'Preparation') || (t[1] != 'SEL2' && i[2] == 'Ende') || (t[1] == 'SEL2' || t[1] == 'SEL4' || t[1] == 'SEL6') && i[2] == 'Hallo' || (t[1] == 'SEL2' && (i[2] == 'Bein' || i[2] == 'schweren' || i[2] == 'Namen')) || (t[1] == 'SEL2' && i[2] == 'Ende' && i[3] == 'Ende') 
        else
      ...
      if !test_transfer[r.measurement.assessment.test_id].nil?
        if !r.responses[0].nil?
          ...
          if r.measurement.assessment.test.shorthand == "SEL6"
            ...
            results = {'V1': sum, 'V2': {'RI':''+sum.to_s + ' von 93', 'KOMS': complex_structure, 'INF': inferenz, 'KO': coherence, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"},'V3': {'SUM':sum ,'RI':''+sum.to_s + ' von 93', 'KOMS': complex_structure, 'INF': inferenz, 'KO': coherence, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"} }
          elsif r.measurement.assessment.test.shorthand == "SEL4"
            ...
            results = {'V1': sum, 'V2': {'RI':''+sum.to_s + ' von 60', 'ADA': ada, 'AVP': avp, 'AVK': avk, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"},'V3': {'SUM':sum ,'RI':''+sum.to_s + ' von 60', 'ADA': ada, 'AVP': avp, 'AVK': avk, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"} }
          elsif r.measurement.assessment.test.shorthand == "SEL2"
            ada = '<strong>Anzahl richtig gelöster Items:</strong> '+ada_cor.to_s+'<hr style="margin-top:0; margin-bottom:0"/>'+ada_cor_items+'<br/><br/><strong>Anzahl falsch gelöster Items:</strong> '+ada_fal.to_s+'<hr style="margin-top:0; margin-bottom:0"/>'+ada_fal_items
            avp = '<strong>Anzahl richtig gelöster Items:</strong> '+avp_cor.to_s+'<hr style="margin-top:0; margin-bottom:0"/>'+avp_cor_items+'<br/><br/><strong>Anzahl falsch gelöster Items:</strong> '+avp_fal.to_s+'<hr style="margin-top:0; margin-bottom:0"/>'+avp_fal_items
            results = {'V1': sum, 'V2': {'RI':''+sum.to_s + ' von 66', 'ADA': ada, 'AVP': avp, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"},'V3': {'SUM':sum ,'RI':''+sum.to_s + ' von 66', 'ADA': ada, 'AVP': avp, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"} }
            ...
            results = {'V1': sum, 'V2': {'RGI': correct, 'FGI': wrong, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"},'V3': {'SUM':sum ,'RGI': correct, 'FGI': wrong, 'LG': (r.total*100).round(1).to_s, 'LGM': "-"} }
          end
